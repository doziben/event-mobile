    ...
    if (valueNotNull) {
      if (multiple) {
        initialSelectedItems =
          Array.isArray(items) &&
          items.filter((item) => value.includes(item[_schema.value]));
      } else {
        initialSelectedItems =
          Array.isArray(items) &&
          items.find((item) => item[_schema.value] === value);
      }
    }
  ...
  useEffect(() => {
    setNecessaryItems((state) => {
      return [...state].map((item) => {
        const _item =
          Array.isArray(items) &&
          items.find((x) => x[_schema.value] === item[_schema.value]);
      ...
      setNecessaryItems((state) => {
        ...
        const newItems = value.reduce((accumulator, currentValue) => {
          ...
          if (index === -1) {
            const item =
              Array.isArray(items) &&
              items.find((item) => item[_schema.value] === currentValue);
